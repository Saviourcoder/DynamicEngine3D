...
namespace DynamicEngine
{
    ...
    private const int historySize = 5;
        ...
        public void GenerateNodesAndBeams(Vector3[] positions, float connectionDistance = -1f, Beam[] beamsArray = null, Transform parent = null)
        {
            ...
            /* 3) Build beams â€" unchanged except we now use the local positions
        ...
        public void Solve()
        {
            ...
            float timeScaleCompensation = 1f;
            if (SceneSettings.Instance != null && SceneSettings.Instance.EnableTimeCompensation)
            {
                float referenceSubSteps = Mathf.Max(1, SceneSettings.Instance.ReferenceSubSteps);
                timeScaleCompensation = (float)numSubSteps / referenceSubSteps;
            }

            float baseTimeScale = SceneSettings.Instance != null ? SceneSettings.Instance.SimulationTimeScale : 1f;
            float compensatedTimeScale = baseTimeScale * timeScaleCompensation;

            float fullDt = Time.fixedDeltaTime * compensatedTimeScale;
        ...
        {
            // Improved collision response values for better ground stability
            const float restitution = 0.02f;        // Lower restitution for less bouncing
            const float friction = 0.9f;            // Higher friction to prevent sliding
            const float velocityDamping = 0.95f;
            const float epsilon = 0.001f;   // Velocity damping for stability
            float dtSquared = dt * dt;

            collisionPoints.Clear();

            // Initialize world space position arrays for XPBD calculations
            // Only initialize if arrays are empty or have mismatched counts
            if (nodeManager.PreviousPositions.Count == 0 || nodeManager.PreviousPositions.Count != nodeManager.Nodes.Count)
            {
                ...
                {
                    // Convert local positions to world space for physics calculations
            ...
            // Step 1: Verlet integration with rest detection (all in world space)
            for (int i = 0; i < nodeManager.Nodes.Count; i++)
            {
            ...
            for (int iter = 0; iter < (SceneSettings.Instance != null ? SceneSettings.Instance.ConstraintIterations : 10); iter++)
            {
                for (int i = 0; i < beams.Count; i++)
                {
                    ...
                    beam.SolveConstraint(dt, nodeMass, nodeMass);
                    ...
                    float wA = nodeManager.IsPinned[beam.nodeA] ? 0f : 1f / nodeMass;
                    float wB = nodeManager.IsPinned[beam.nodeB] ? 0f : 1f / nodeMass;
                    float wSum = wA + wB;
            ...
            for (int i = 0; i < nodeManager.Nodes.Count; i++)
            {
                ...
                for (int j = 0; j < hitCount; j++)
                {
                    ...
                    if (distance < collisionRadius && distance > 0.001f)
                    {
                        ...
                        // Normal impulse
                        float jn = -(1 + restitution) * normalVelocity;
                        jn /= (1f / nodeMass + 1f / otherSolver.nodeMass);
                        ...
                        jt /= (1f / nodeMass + 1f / otherSolver.nodeMass);
        ...
        public void ApplyForceToNode(int nodeIndex, Vector3 force)
        {
            ...
            {
                // Force should be in world space for proper physics calculations
                Vector3 worldForce = owner.TransformDirection(force); // Convert from local to world space if needed
                Vector3 acceleration = worldForce / nodeMass;
                
                // Get current world position
                Vector3 currentWorldPos = owner.TransformPoint(nodeManager.Nodes[nodeIndex].localPosition);
                
                // Apply force by modifying previous position (for Verlet integration)
                nodeManager.PreviousPositions[nodeIndex] = currentWorldPos - acceleration * Time.fixedDeltaTime * Time.fixedDeltaTime;
            }
        }
        ...
        public void ApplyImpulse(int nodeIndex, Vector3 targetPosition, float strength)
        {
            if (nodeIndex >= 0 && nodeIndex < nodeManager.Nodes.Count && nodeManager.Nodes[nodeIndex] != null && !nodeManager.IsPinned[nodeIndex])
            {
                ...
                Vector3 impulse = delta * strength * nodeMass / Time.fixedDeltaTime;
        ...
        private void ApplyPressureForces(float deltaTime)
        {
            ...
            float mNode = Mathf.Max(1e-6f, nodeMass);      // kg
