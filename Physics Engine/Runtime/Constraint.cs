...
namespace DynamicEngine
{
    ...
    public class Constraint : MonoBehaviour
    {
        ...
        private void InitializeBeams()
        {
            ...
            // Log constraint initialization details
            Debug.Log($"[Constraint] Initializing: '{name}' connecting '{m_attachedBody.name}' to '{m_baseBody.name}'");
            Debug.Log($"[Constraint] Attached Body: {m_attachedBody.solver.nodeManager.Nodes.Count} nodes");
            Debug.Log($"[Constraint] Base Body: {m_baseBody.solver.nodeManager.Nodes.Count} nodes");
            Debug.Log($"[Constraint] Configuring {m_links.Count} link(s)");
            ...
            Debug.Log($"[Constraint] Initialization complete: {successfulLinks} beams created from {m_links.Count} link configs");
        }
        private void InitializeWorldConstraints()
        {
            ...
            Debug.Log($"[Constraint] Initializing world constraints for '{name}'");
            Debug.Log($"[Constraint] Configuring {m_links.Count} world link(s)");
            int successfulLinks = 0;
            for (int i = 0; i < m_links.Count; i++)
            {
                var config = m_links[i];
        ...
        private void CreateNodeToNodeBeam(int nodeA, int nodeB, LinkConfig config, float restLength, ref int successfulLinks, int linkIndex)
        {
            ...
            plasticityThreshold: m_attachedBody.solver.plasticityThreshold,
            plasticityRate: m_attachedBody.solver.plasticityRate,
            ...
            successfulLinks++;
            Debug.Log($"[Constraint] Link {linkIndex}: Created Node-to-Node Beam from node {nodeA} to node {nodeB}");
        }
