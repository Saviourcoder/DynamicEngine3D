...
namespace DynamicEngine
{
    ...
    public class Solver
    {
        ...
        {
            float restitutionValue = matterAsset != null ? matterAsset.Restitution : 0.02f;
            ...
            float dampingValue = matterAsset != null ? matterAsset.Damping : 0.95f;
            ...
            for (int i = 0; i < nodeManager.Nodes.Count; i++)
            {
                ...
                float dampingPerSecond = 1f - dampingValue; // Tie to damping value (e.g., 0.05 for 0.95)
            ...
            for (int iter = 0; iter < constraintIterations; iter++)
            {
                for (int i = 0; i < beams.Count; i++)
                {
                    ...
                    // If strain exceeds plasticity threshold, permanently deform the beam
                    if (strain > beam.plasticityThreshold)
                    {
                        // DEBUG: Log entry into plasticity check
                        if (iter == 0 && i % 10 == 0)
                        {
                            Debug.Log($"[PLASTICITY DEBUG] Beam {i} (Iter {iter}): Entering plasticity - Strain={strain:F4} > Threshold={beam.plasticityThreshold:F4}");
                        }
                        ...
                        // DEBUG: Log calculated plastic deformation
                        if (iter == 0 && i % 10 == 0)
                        {
                            Debug.Log($"[PLASTICITY DEBUG] Beam {i}: ExcessStrain={excessStrain:F4}, PlasticDeformation={plasticDeformation:F4} (Rate={beam.plasticityRate:F4}, dt={dt:F4})");
                        }
                        ...
                        {
                            float oldRestLength = beam.restLength;

                            // DEBUG: Log before change
                            if (iter == 0 && i % 10 == 0)
                            {
                                Debug.Log($"[PLASTICITY DEBUG] Beam {i}: Before Change - RestLength={oldRestLength:F4}, Original={beam.originalRestLength:F4}");
                            }

                            // Apply plastic deformation by permanently changing rest length
                            float deformationAmount = plasticDeformation * beam.restLength;  // Use beam.restLength instead of worldRestLength for consistency
                            ...
                            // Update total deformation after change
                            totalDeformation = Mathf.Abs(beam.restLength - beam.originalRestLength) / beam.originalRestLength;

                            // LOG: Track rest length changes (moved after update)
                            float restLengthChange = beam.restLength - oldRestLength;
                            if (Mathf.Abs(restLengthChange) > 0.0001f && iter == 0 && i % 10 == 0)
                            {
                                Debug.Log($"[PLASTICITY] Beam {i}: Strain={strain:F4}, Threshold={beam.plasticityThreshold:F4}, " +
                                          $"TotalDeformation={totalDeformation:F4}/{beam.maxDeformation:F4} " +
                                          $"(Change: {restLengthChange:F6}, Original: {beam.originalRestLength:F4})");
                            }

                            // DEBUG: Log after change and clamp
                            if (iter == 0 && i % 10 == 0)
                            {
                                Debug.Log($"[PLASTICITY DEBUG] Beam {i}: After Change - RestLength={beam.restLength:F4}, Change={restLengthChange:F6}, Clamped={(beam.restLength == minRestLength || beam.restLength == maxRestLength)}");
                            }

                            // Update world rest length for current iteration
                            localRestVector = Vector3.right * beam.restLength;
                            worldRestVector = owner.TransformDirection(localRestVector);
                            ...
                            // DEBUG: Log recalculated constraint
                            if (iter == 0 && i % 10 == 0)
                            {
                                Debug.Log($"[PLASTICITY DEBUG] Beam {i}: Recalculated - Constraint={constraint:F4}, WorldRestLength={worldRestLength:F4}");
                            }
                        }
                        else
                        {
                            // LOG: Max deformation reached
                            if (iter == 0 && i % 20 == 0)
                            {
                                Debug.LogWarning($"[PLASTICITY] Beam {i} reached MAX DEFORMATION: {totalDeformation:F4}/{beam.maxDeformation:F4}");
                            }

                            // DEBUG: Log when max deformation blocks change
                            if (iter == 0 && i % 10 == 0)
                            {
                                Debug.Log($"[PLASTICITY DEBUG] Beam {i}: Skipped Change - TotalDeformation={totalDeformation:F4} >= Max={beam.maxDeformation:F4}");
                            }
                        }
                    }
                    ...
                    if (visualizeForces && constraint != 0)
                    {
                        ...
                        Color forceColor = strain > beam.plasticityThreshold ? Color.magenta : (forceMagnitude > 0 ? Color.red : Color.blue);
        ...
        public void ApplyForceToNode(int nodeIndex, Vector3 force)
        {
            ...
            float effectiveDtSquared = (fullDt * fullDt) / Mathf.Max(1, subSteps);
            ...
            // Use effectiveDtSquared instead of fullDt*fullDt
            nodeManager.PreviousPositions[nodeIndex] = currentWorldPos - accel * effectiveDtSquared;
        }
