...
namespace DynamicEngine.Editor
{
    ...
    public class ConstraintEditor : UnityEditor.Editor
    {
        ...
        private SerializedProperty isWheelProp;
        ...
        protected virtual void OnEnable()
        {
            ...
            isWheelProp = serializedObject.FindProperty("m_isWheel");
        ...
        {
            // Never recalculate rest lengths during play mode — the SoftBody transform
            // is moved to the CoM each frame, so TransformPoint(truss.NodePositions[i])
            // produces wrong world positions and corrupts live beam rest lengths.
        ...
        public override void OnInspectorGUI()
        {
            ...
            SoftBody softBodyA = (m_targets.Length == 1) ? m_targets[0].GetComponent<SoftBody>() : null;
            ...
            if (softBodyB != null)
            {
                ...
                EditorGUILayout.PropertyField(isWheelProp, new GUIContent("Is Wheel", "Mark this constraint as a wheel so CarController includes it in the drive system."));
                ...
                if (enableMotorProp.boolValue)
                {
                    ...
                    if (m_targets.Length == 1)
                    {
                        ...
                        Constraint targetConstraint = m_targets[0];
                        string[] axisNodes = GetNodes(softBodyA, 2, 2); // Exactly 2 nodes for axis, from ATTACHED body
                    ...
                    {
                        EditorGUILayout.HelpBox("Motor requires axis nodes to be set! Select a NodeSet with exactly 2 nodes from this body that define the rotation axis.", MessageType.Warning);
                    }
