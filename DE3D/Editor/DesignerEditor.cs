...
{
    [CustomEditor(typeof(DynaDesigner3D))]
    public class DesignerEditor : UnityEditor.Editor
    {
        ...
        #region References
        private DynaDesigner3D designer;
        ...
        {
            designer = (DynaDesigner3D)target;
        ...
        public override void OnInspectorGUI()
        {
            ...
            // Track changes for the entire inspector
            ...
            // Save changes to the designer instance
            if (EditorGUI.EndChangeCheck() || GUI.changed)
            {
                EditorUtility.SetDirty(designer);
                if (designer.autoApplyChanges)
                {
                    designer.SyncDesignerToSolver();
                }
        ...
        private new void DrawHeader()
        {
            ...
            EditorGUILayout.LabelField("DynaDesigner3D", headerStyle);
            EditorGUILayout.Space(5);
        }
        ...
        private void DrawBeamsTab()
        {
            ...
            EditorGUI.BeginChangeCheck();
            designer.selectedBeamPreset = (DynaDesigner3D.BeamPreset)EditorGUILayout.EnumPopup("Preset", designer.selectedBeamPreset);
            if (EditorGUI.EndChangeCheck() && designer.selectedBeamPreset != DynaDesigner3D.BeamPreset.Custom)
            {
                ApplyBeamPreset(designer.selectedBeamPreset);
            }

            EditorGUI.BeginChangeCheck();
            // Stiffness is displayed as 1/compliance for user friendliness.
            // Internally beams store compliance; we convert on write.
            designer.defaultBeamStiffness = EditorGUILayout.FloatField("Stiffness", designer.defaultBeamStiffness);
            designer.defaultBeamDamping = EditorGUILayout.FloatField("Damping", designer.defaultBeamDamping);
            ...
            // ── Bulk Multipliers ─────────────────────────────────────────────
            designer.showBeamMultipliers = EditorGUILayout.Foldout(
                designer.showBeamMultipliers, "Bulk Multipliers", true, EditorStyles.foldoutHeader);

            if (designer.showBeamMultipliers)
            {
                EditorGUI.indentLevel++;

                string scopeLabel = count > 0 ? $"Applies to {count} selected beam{(count == 1 ? "" : "s")}"
                                               : "Applies to all beams (none selected)";
                EditorGUILayout.HelpBox(scopeLabel, MessageType.None);

                EditorGUILayout.Space(4);
                EditorGUILayout.LabelField("Scale each property by a percentage of its current value.", EditorStyles.miniLabel);
                EditorGUILayout.Space(4);

                designer.beamStiffnessMultiplier = EditorGUILayout.Slider("Stiffness %", designer.beamStiffnessMultiplier, 1f, 500f);
                designer.beamDampingMultiplier = EditorGUILayout.Slider("Damping %", designer.beamDampingMultiplier, 1f, 500f);
                designer.beamPlastThreshMultiplier = EditorGUILayout.Slider("Plasticity Thresh %", designer.beamPlastThreshMultiplier, 1f, 500f);
                designer.beamPlastRateMultiplier = EditorGUILayout.Slider("Plasticity Rate %", designer.beamPlastRateMultiplier, 1f, 500f);
                designer.beamMaxDefMultiplier = EditorGUILayout.Slider("Max Deformation %", designer.beamMaxDefMultiplier, 1f, 500f);

                EditorGUILayout.Space(6);

                EditorGUILayout.BeginHorizontal();

                if (GUILayout.Button("Apply", GetRoundedButtonStyle(), GUILayout.Height(28)))
                    ApplyBeamMultipliers();

                if (GUILayout.Button("Reset to 100%", GetRoundedButtonStyle(), GUILayout.Height(28)))
                {
                    designer.beamStiffnessMultiplier = 100f;
                    designer.beamDampingMultiplier = 100f;
                    designer.beamPlastThreshMultiplier = 100f;
                    designer.beamPlastRateMultiplier = 100f;
                    designer.beamMaxDefMultiplier = 100f;
                }
        ...
        private void ExecuteDeleteShortcut()
        {
            ...
            {
                case DynaDesigner3D.DesignerMode.Node:
                    if (designer.selectedNodes.Count > 0) DeleteSelectedNodes();
                    break;
                case DynaDesigner3D.DesignerMode.Beam:
                    if (designer.selectedBeams.Count > 0) DeleteSelectedBeams();
                    break;
                case DynaDesigner3D.DesignerMode.Face:
        ...
        {
            DynaDesigner3D.DesignerMode previousMode = designer.currentMode;

            switch (currentTab)
            {
                case TAB_NODES:
                case TAB_NODESETS:
                    designer.currentMode = DynaDesigner3D.DesignerMode.Node;
                    break;
                case TAB_BEAMS:
                case TAB_LINKSETS:
                    designer.currentMode = DynaDesigner3D.DesignerMode.Beam;
                    break;
                case TAB_FACES:
                    designer.currentMode = DynaDesigner3D.DesignerMode.Face;
                    break;
                default:
                    designer.currentMode = DynaDesigner3D.DesignerMode.Node;
                    break;
            ...
            if (previousMode == DynaDesigner3D.DesignerMode.Node && designer.currentMode != DynaDesigner3D.DesignerMode.Node)
            {
                if (isCreatingNode)
                {
        ...
        private void ApplyBeamPreset(DynaDesigner3D.BeamPreset preset)
        {
            var (c, d, thresh, rate, maxDef) = GetBeamPresetValues(preset);

            designer.defaultBeamStiffness = c > 0f ? 1f / c : 100f;
        ...
        private void ApplyBeamPropertiesToSelection()
        {
            ...
            foreach (int idx in designer.selectedBeams)
            {
                if (idx < beams.Count)
                {
                    beams[idx].compliance = designer.defaultBeamStiffness > 0f ? 1f / designer.defaultBeamStiffness : 0f;
            ...
            // NEW: Push changes live to the running physics solver
            if (Application.isPlaying && targetSoftBody != null && targetSoftBody.solver != null)
            {
                var liveBeams = targetSoftBody.solver.beams;
                foreach (int idx in designer.selectedBeams)
                {
                    if (idx < liveBeams.Count)
                    {
                        var liveBeam = liveBeams[idx];
                        liveBeam.compliance = designer.defaultBeamStiffness > 0f ? 1f / designer.defaultBeamStiffness : 0f;
                        liveBeam.damping = designer.defaultBeamDamping;
                        liveBeam.plasticityThreshold = designer.defaultPlasticityThreshold;
                        liveBeam.plasticityRate = designer.defaultPlasticityRate;
                        liveBeam.maxDeformation = designer.defaultMaxDeformation;
                    }
                }
            }
        }
        private void ApplyBeamMultipliers()
        {
            var beams = workingTruss.GetTrussBeams();
            if (beams == null || beams.Count == 0) return;

            // Decide which beam indices to modify
            IEnumerable<int> targets = designer.selectedBeams.Count > 0
                ? designer.selectedBeams
                : Enumerable.Range(0, beams.Count);

            float sMul = designer.beamStiffnessMultiplier / 100f;   
            float dMul = designer.beamDampingMultiplier / 100f;
            float ptMul = designer.beamPlastThreshMultiplier / 100f;
            float prMul = designer.beamPlastRateMultiplier / 100f;
            float mdMul = designer.beamMaxDefMultiplier / 100f;

            foreach (int idx in targets)
            {
                if (idx >= beams.Count) continue;
                var b = beams[idx];
                // Dividing compliance by sMul is equivalent to multiplying stiffness by sMul
                b.compliance = (sMul > 0f && b.compliance > 0f) ? b.compliance / sMul : b.compliance;
                b.damping = Mathf.Max(0f, b.damping * dMul);
                b.plasticityThreshold = Mathf.Max(0f, b.plasticityThreshold * ptMul);
                b.plasticityRate = Mathf.Max(0f, b.plasticityRate * prMul);
                b.maxDeformation = Mathf.Max(0f, b.maxDeformation * mdMul);
            }

            EditorUtility.SetDirty(workingTruss);
            if (designer.autoApplyChanges) ApplyTrussToTarget();

            // Push live if in play mode
            if (Application.isPlaying && targetSoftBody != null && targetSoftBody.solver != null)
            {
                var liveBeams = targetSoftBody.solver.beams;
                foreach (int idx in targets)
                {
                    if (idx >= beams.Count || idx >= liveBeams.Count) continue;
                    liveBeams[idx].compliance = beams[idx].compliance;
                    liveBeams[idx].damping = beams[idx].damping;
                    liveBeams[idx].plasticityThreshold = beams[idx].plasticityThreshold;
                    liveBeams[idx].plasticityRate = beams[idx].plasticityRate;
                    liveBeams[idx].maxDeformation = beams[idx].maxDeformation;
                }
            }

            // Refresh inspector fields from first affected beam
            UpdateBeamUIFromSelection();
        }

        private void UpdateBeamUIFromSelection()
        {
            if (designer.selectedBeams.Count > 0 && workingTruss != null)
            {
                var beams = workingTruss.GetTrussBeams();
                int firstSelected = designer.selectedBeams[0];

                if (firstSelected < beams.Count)
                {
                    var beam = beams[firstSelected];
                    designer.defaultBeamStiffness = beam.compliance > 0f ? 1f / beam.compliance : 0f;
                    designer.defaultBeamDamping = beam.damping;
                    designer.defaultPlasticityThreshold = beam.plasticityThreshold;
                    designer.defaultPlasticityRate = beam.plasticityRate;
                    designer.defaultMaxDeformation = beam.maxDeformation;
        ...
        private (float c, float d, float thresh, float rate, float maxDef) GetBeamPresetValues(DynaDesigner3D.BeamPreset preset)
        {
            switch (preset)
            {
                case DynaDesigner3D.BeamPreset.Metal:
                    return (0.001f, 0.1f, 0.05f, 0.1f, 0.2f);
                case DynaDesigner3D.BeamPreset.Rubber:
        ...
        private void CreateBeam(int a, int b)
        {
            ...
            workingTruss.GetTrussBeams().Add(new Beam(
                a, b,
                designer.defaultBeamStiffness > 0f ? 1f / designer.defaultBeamStiffness : 0f,
            ...
            for (int i = 0; i < designer.selectedNodes.Count; i++)
            {
                for (int j = i + 1; j < designer.selectedNodes.Count; j++)
                {
                    ...
                    if (!exists)
                    {
                        ...
                        beams.Add(new Beam(nodeA, nodeB,
                                         designer.defaultBeamStiffness > 0f ? 1f / designer.defaultBeamStiffness : 0f,
                                         designer.defaultBeamDamping, Vector3.Distance(pA, pB),
                                         designer.defaultMaxDeformation,
                                         designer.defaultPlasticityThreshold,
                                         designer.defaultPlasticityRate));
                        created++;
                    }
                }
            }
