...
namespace DynamicEngine
{
    ...
    public class TrussAssetEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            SerializedProperty linkSetsProp = serializedObject.FindProperty("linkSets");
            ...
            for (int i = 0; i < linkSetsProp.arraySize; i++)
            {
                EditorGUILayout.BeginVertical(GUI.skin.box);
                SerializedProperty linkSetProp = linkSetsProp.GetArrayElementAtIndex(i);
                EditorGUILayout.PropertyField(linkSetProp.FindPropertyRelative("name"), new GUIContent("Name"));
                EditorGUILayout.PropertyField(linkSetProp.FindPropertyRelative("linkIndices"), new GUIContent("Link Indices"), true);
                EditorGUILayout.PropertyField(linkSetProp.FindPropertyRelative("color"), new GUIContent("Color"));
                EditorGUILayout.PropertyField(linkSetProp.FindPropertyRelative("isVisible"), new GUIContent("Is Visible"));
                if (GUILayout.Button("Remove Link Set"))
                {
                    linkSetsProp.DeleteArrayElementAtIndex(i);
                    serializedObject.ApplyModifiedProperties();
                    EditorUtility.SetDirty(truss);
                    return;
                }
            ...
            EditorGUILayout.Space();
            if (GUILayout.Button("Add New Link Set", GUILayout.Height(30)))
            {
                linkSetsProp.arraySize++;
                SerializedProperty newLinkSet = linkSetsProp.GetArrayElementAtIndex(linkSetsProp.arraySize - 1);
                newLinkSet.FindPropertyRelative("name").stringValue = $"LinkSet{linkSetsProp.arraySize}";
                newLinkSet.FindPropertyRelative("linkIndices").arraySize = 0;
                newLinkSet.FindPropertyRelative("color").colorValue = Color.magenta;
                newLinkSet.FindPropertyRelative("isVisible").boolValue = true;
                serializedObject.ApplyModifiedProperties();
                EditorUtility.SetDirty(truss);
            }
