...
namespace DynamicEngine
{
    ...
    public struct LinkConfig
    {
        ...
        public float stiffness;      // Now used as spring constant (k), higher = stiffer. Use 'infinity' for completely rigid constraint.
        public bool master;
        public bool show;
    }
    ...
    public class Constraint : MonoBehaviour
    {
        ...
        // Track disabled collision pairs for cleanup
        ...
        private Vector3 m_lastAxisCenter = Vector3.zero;
        ...
        {
            if (!m_enableMotor || m_attachedBody == null || m_attachedBody.solver == null)
                return;
            ...
            {
                Debug.LogWarning($"Motor requires at least 2 nodes in axis set '{m_axisNodeSet}'");
                return;
            }
            ...
            Vector3 currentCenter = axisCenter;
            Vector3 displacement = currentCenter - m_lastAxisCenter;
            float angularDisplacement = Vector3.Dot(Vector3.Cross(displacement, axisDirection), axisDirection);
            m_currentAngularVelocity = angularDisplacement / dt;
            m_lastAxisCenter = currentCenter;

            // Calculate target angular velocity in radians/sec
            ...
            // Calculate required torque (clamped to max)
            float desiredTorque = angularError * 10f; // Proportional gain
            float appliedTorque = Mathf.Clamp(desiredTorque, -m_maxTorque, m_maxTorque);

            // Apply torque as forces to attached body nodes
            ...
            for (int i = 0; i < attachedNodeCount; i++)
            {
                ...
                // Vector from axis center to node
                Vector3 radius = nodePos - axisCenter;

                // Project radius onto plane perpendicular to axis
                ...
                // Force magnitude = torque / radius
                float forceMagnitude = appliedTorque / (distance * attachedNodeCount);

                // Apply force
                Vector3 force = tangent * forceMagnitude;
        ...
        void LateUpdate()
        {
            ...
            if (!m_beamsInitialized && Application.isPlaying)
            {
                ...
                m_lastAxisCenter = GetAxisCenter(); // Initialize for motor
            }
        ...
        private void SetupCollisionDisabling()
        {
            ...
            // Get all colliders from both bodies
            ...
            // Disable collision between all pairs
        ...
        private void InitializeBeams()
        {
            ...
            // Set up collision disabling if enabled
        ...
        private void CreateNodeToNodeBeam(int nodeA, int nodeB, LinkConfig config, float restLength, ref int successfulLinks, int linkIndex)
        {
            ...
            // Check if stiffness is infinity or extremely high for rigid constraint
            bool isInfiniteStiffness = float.IsInfinity(config.stiffness) || config.stiffness > 1e8f;
            float baseCompliance = (isRigid || isInfiniteStiffness) ? 0f : (config.stiffness > 0f ? 1f / config.stiffness : 1e-6f);
            int parallelCount = 1;
        ...
        private void OnValidate()
        {
            if (m_links != null)
            {
                for (int i = 0; i < m_links.Count; i++)
                {
                    ...
                    // Allow infinity for stiffness (rigid constraint), otherwise clamp to minimum of 1
                    if (!float.IsInfinity(config.stiffness))
                    {
