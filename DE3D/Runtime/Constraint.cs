    ...
    public class Constraint : MonoBehaviour
    {
        ...
        private void ApplyMotorTorque(float dt)
        {
            ...
            for (int i = 0; i < attachedNodeCount; i++)
            {
                ...
                // ApplyWorldForceToNode is called once per FixedUpdate but uses substep dt,
                // so the Verlet velocity delta it produces is (force/mass)*substepDt.
                // We want (force/mass)*fixedDeltaTime worth of Δv over the full step,
                // so multiply the force by subSteps to compensate.
                // The customDt passed below must still be substepDt so the Verlet
                // prevPos encoding/decoding stays self-consistent.
                int subSteps = (SceneSettings.Instance != null) ? SceneSettings.Instance.SubstepCount : 1;
                float forceMag = appliedTorque * subSteps / (distance * attachedNodeCount);
        ...
        private void SetupCollisionDisabling()
        {
            ...
            // --- Unity physics colliders (only relevant when both bodies have them) ---
            ...
            // --- Custom solver collision handler (always applied regardless of Unity colliders) ---
            // Hard-exclude so the broad phase (AABB / BVH) never starts for this pair.
