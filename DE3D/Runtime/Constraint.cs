...
namespace DynamicEngine
{
    ...
    public struct LinkConfig
    {
        ...
        [Tooltip("If Min/Max are zero, this defines the target length. If 0, defaults to initial distance.")]
    ...
    public class Constraint : MonoBehaviour
    {
        ...
        private float DetermineTargetRestLength(LinkConfig config, Vector3 posA, Vector3 posB)
        {
            ...
            if (config.minLength > 0f || config.maxLength > 0f)
            {
                ...
                // Use Infinity if maxLength is 0 to allow unlimited expansion
        ...
        private void UpdateBeamPropertiesFromLinks()
        {
            ...
            for (int i = 0; i < m_links.Count; i++)
            {
                ...
                foreach (var beam in beams)
                {
                    ...
                    // RECALCULATE TARGETS: This fixes the MaxLength "no effect" bug
                    ...
                    {
                        // We must refresh the target based on the current range to let the solver react
                        float min = config.minLength;
                        float max = config.maxLength > 0f ? config.maxLength : float.PositiveInfinity;

                        if (beam.isEdgeSliding)
                        {
                            // For edge sliding, we need to re-project to find current perp distance
                        ...
                        {
                            // For standard beams, clamp the current target to the new range
        ...
        private void InitializeWorldConstraints()
        {
            ...
            for (int i = 0; i < m_links.Count; i++)
            {
                ...
                {
                    // World constraints are essentially beams attached to a static point in space
                    // Assuming your Beam class handles bodyB = null as a world-space anchor:
                    Vector3 worldAnchor = m_attachedBody.solver.nodeManager.PredictedPositions[nodeIndex];

                    Beam worldBeam = new Beam(
                        nodeA: nodeIndex,
                        nodeB: -1, // Convention for world anchor
                        compliance: 0f,
                        damping: 0f,
                        restLength: config.restLength > 0 ? config.restLength : 0f,
                        maxDeformation: m_attachedBody.solver.maxDeformation,
                        bodyA: m_attachedBody,
                        bodyB: null // No second body
                    );
        ...
        {
            // ONLY RUN DURING RUNTIME (Editor handles edit-mode visualization)
            if (!Application.isPlaying) return;
