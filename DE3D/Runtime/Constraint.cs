...
namespace DynamicEngine
{
    ...
    public class Constraint : MonoBehaviour
    {
        ...
        [SerializeField] private bool m_isWheel = false;
        ...
        private bool m_collisionDisablingSetup = false;
        ...
        public bool enableMotor { get => m_enableMotor; set => m_enableMotor = value; }
        public string axisNodeSet => m_axisNodeSet;
        public float targetRate { get => m_targetRate; set => m_targetRate = value; }
        public float maxTorque { get => m_maxTorque; set => m_maxTorque = value; }
        public bool isWheel { get => m_isWheel; }
        ...
        {
            if (m_attachedBody == null || m_attachedBody.solver == null) return Vector3.zero;
            ...
            {
                center += m_attachedBody.solver.nodeManager.PredictedPositions[idx];
            }
        ...
        {
            if (m_attachedBody == null || m_attachedBody.solver == null) return Vector3.up;

            int[] axisIndices = GetAxisNodeIndices();
            if (axisIndices == null || axisIndices.Length < 2) return Vector3.up;

            Vector3 posA = m_attachedBody.solver.nodeManager.PredictedPositions[axisIndices[0]];
            Vector3 posB = m_attachedBody.solver.nodeManager.PredictedPositions[axisIndices[1]];

            Vector3 axis = (posB - posA).normalized;
            return axis.magnitude > 0.001f ? axis : Vector3.up;
        }

        private float GetCurrentDt()
        {
            if (m_attachedBody?.solver == null || SceneSettings.Instance == null)
                return Time.fixedDeltaTime;

            int subSteps = SceneSettings.Instance.SubstepCount;
            return Time.fixedDeltaTime / Mathf.Max(1, subSteps);
        }
        ...
        private void ApplyMotorTorque(float dt)
        {
            ...
            int validNodes = 0;
            int attachedNodeCount = m_attachedBody.solver.nodeManager.NodeCount;
            ...
            for (int i = 0; i < attachedNodeCount; i++)
            {
                ...
                if (m_lastNodeAngles.ContainsKey(i))
                {
                    ...
                    // Use fixedDeltaTime here, not the substep dt.
                    // m_lastNodeAngles is only updated once per FixedUpdate, so the elapsed
                    // window between samples is always the full physics step — not one substep.
                    // Dividing by substep dt was inflating the measured ω by SubstepCount,
                    // making the controller think the wheel was already at max speed.
                    totalAngularVelocity += deltaAngle / Time.fixedDeltaTime;
                    validNodes++;
                }
        ...
        public void KillRotationalVelocity()
        {
            if (m_attachedBody?.solver == null) return;

            var nm = m_attachedBody.solver.nodeManager;
            Vector3 axisCenter = GetAxisCenter();
            ...
            {
                if (nm.IsPinned[i]) continue;

                Vector3 pos = nm.PredictedPositions[i];
                Vector3 vel = (pos - nm.PreviousPositions[i]) / dt;

                Vector3 radial = (pos - axisCenter);
                radial -= Vector3.Project(radial, axisDir);

                if (radial.magnitude < 0.001f) continue;

                Vector3 tangent = Vector3.Cross(axisDir, radial).normalized;
                float tangentialSpeed = Vector3.Dot(vel, tangent);

                vel -= tangent * tangentialSpeed;
                nm.PreviousPositions[i] = pos - vel * dt;
            }
        }
        ...
        private void FixedUpdate()
        {
            ...
            if (m_disableCollision && !m_collisionDisablingSetup && m_baseBody != null)
        ...
        private void SetupCollisionDisabling()
        {
            ...
            RestoreCollisionPairs(); // still needed on re-setup
            ...
            {
                m_attachedBody.solver.collisionHandler.SetIgnoreCollision(m_attachedBody.transform, m_baseBody.transform, true);
                m_attachedBody.solver.collisionHandler.HardExcludeBody(m_baseBody.transform);
            }

            if (m_baseBody.solver?.collisionHandler != null)
            {
                m_baseBody.solver.collisionHandler.SetIgnoreCollision(m_attachedBody.transform, m_baseBody.transform, true);
                m_baseBody.solver.collisionHandler.HardExcludeBody(m_attachedBody.transform);
            }

            m_collisionDisablingSetup = true; 
        }
        ...
        private void RestoreCollisionPairs()
        {
            ...
            if (m_baseBody != null)
            {
                if (m_attachedBody?.solver?.collisionHandler != null)
                {
                    ...
                    m_attachedBody.solver.collisionHandler.RemoveHardExcludedBody(m_baseBody.transform);
                }
                ...
                if (m_baseBody.solver?.collisionHandler != null)
                {
                    m_baseBody.solver.collisionHandler.SetIgnoreCollision(
                        m_attachedBody.transform, m_baseBody.transform, false);
                    m_baseBody.solver.collisionHandler.RemoveHardExcludedBody(m_attachedBody.transform);
                }
            }
            m_collisionDisablingSetup = false;
        }
        ...
        private void InitializeBeams()
        {
            ...
            if (m_attachedBody.solver?.nodeManager == null || m_baseBody.solver?.nodeManager == null)
            {
        ...
        private bool ValidateAttachment(int[] indices, AttachmentType type, string bodyLabel, int linkIndex, SoftBody body)
        {
            ...
            {
                if (index < 0 || index >= body.solver.nodeManager.NodeCount)
                {
                    Debug.LogWarning($"Invalid node index {index} for link {linkIndex} body {bodyLabel}. Nodes={body.solver.nodeManager.NodeCount}. Skipping.", this);
                    return false;
                }
            }
            return true;
        ...
        private Vector3 GetProjectedPointOnEdge(SoftBody body, int[] edgeIndices, Vector3 point)
        {
            ...
            int maxIndex = useRuntime ? body.solver.nodeManager.NodeCount : body.truss.NodePositions.Length;
