...
public class CarController : MonoBehaviour
{
    ...
    // All SoftBody instances in the scene — populated at Start so the grabber
    // can show nodes for every body, not just the car frame.
    private SoftBody[] _allSoftBodies = new SoftBody[0];
    ...
    private void FindInteriorCamera()
    {
        ...
        if (cams.Length > 0) 
        {
           interiorCamera = cams[0].GetComponent<Camera>();
        }

        if (interiorCamera == null)
        {
            Debug.Log("No Interior Camera found in Scene");
        }
    }
    ...
    private void UpdateThrottle()
    {
        ...
        else if (Mathf.Abs(throttle) > 0.01f)
        {
            ...
            else
            {
                ...
                // Removed * currentGearRatio from active torque calculation
        ...
        else
        {
            ...
            // Removed * currentGearRatio from coasting/engine braking calculation
            engineTorque = (1f + engineTargetRPM / maxRPM);
        ...
        foreach (Constraint wheel in wheels)
        {
            ...
            if (handbrake)
            {
                ...
                if (!isFront)
                {
                    wheel.KillRotationalVelocity();
                    SetConstraintMotor(wheel, 0f, currentBrakeTorque);
                }
    ...
    private void HandleNodeGrabbing()
    {
        ...
        {
            // Re-scan the scene every frame grab is active so any SoftBody that
            // was spawned or initialized after Start() is never missed.
            _allSoftBodies = FindObjectsByType<SoftBody>(FindObjectsSortMode.None);

            foreach (SoftBody sb in _allSoftBodies)
                if (sb != null && !grabVisualsContainers.ContainsKey(sb))
                    SetupNodeGrabVisuals(sb);
            ...
            foreach (SoftBody sb in _allSoftBodies)
            {
                if (sb == null || sb.solver?.nodeManager == null) continue;
                for (int i = 0; i < sb.solver.nodeManager.NodeCount; i++)
                {
                    Vector3 nodePos = sb.solver.nodeManager.GetPosition(i);
            ...
            // Update visuals for every SoftBody.
            foreach (SoftBody sb in _allSoftBodies)
            {
                if (sb == null) continue;
                int hoverIdx = (sb == hoveredSoftBody) ? hoveredNodeIndex : -1;
                int dragIdx = (sb == draggedSoftBody) ? draggedNodeIndex : -1;
                Vector3 targ = (sb == draggedSoftBody) ? grabTargetPosition : Vector3.zero;
                UpdateNodeGrabVisuals(sb, true, hoverIdx, dragIdx, targ);
            }
        }
        else
        {
            // Ctrl released — hide visuals on all bodies and cancel any drag.
            foreach (SoftBody sb in _allSoftBodies)
                if (sb != null) UpdateNodeGrabVisuals(sb, false, -1, -1, Vector3.zero);
