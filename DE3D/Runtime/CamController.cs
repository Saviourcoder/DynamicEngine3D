...
namespace DynamicEngine
{
    ...
    public class CameraVisualizerController : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            if (nm == null || draggedNodeIndex >= nm.NodeCount)
                return;

            float blend = Mathf.Clamp01(nodeGrabStiffness * Time.fixedDeltaTime);
            _smoothedGrabTarget = Vector3.Lerp(_smoothedGrabTarget, targetPosition, blend);
            ...
            // 1. Calculate current implicit velocity from the Verlet system
            Vector3 currentVelocity = (current - nm.PreviousPositions[draggedNodeIndex]) / Time.fixedDeltaTime;

            // 2. Apply a critically damped spring force
            float k = nodeGrabForce;
            float damping = Mathf.Sqrt(k) * 2f;
            Vector3 springForce = (toTarget * k) - (currentVelocity * damping);

            // 3. Update velocity and inject back into PreviousPositions
            Vector3 newVelocity = currentVelocity + (springForce * Time.fixedDeltaTime);
            nm.PreviousPositions[draggedNodeIndex] = current - (newVelocity * Time.fixedDeltaTime);

            lastGrabForceMag = springForce.magnitude;
        }
        ...
        private void HandleNodeGrabbing()
        {
            ...
            if (isControlPressed)
            {
                    ...
                    draggedNodeIndex < draggedSoftBody.solver.nodeManager.NodeCount)
                {
                    ...
                    {
                        Vector3 currentNodePos = draggedSoftBody.solver.nodeManager.GetPosition(draggedNodeIndex);
        ...
        private void UpdateSoftBodyVisuals(SoftBody sb, bool show, int hoverIdx, int dragIdx, Vector3 targPos)
        {
            ...
            for (int i = 0; i < nodeCount; i++)
            {
                ...
                bool isVisible = !hasCenter || Vector3.Distance(nodePos, nodes[centerIdx]) <= nodeVisualShowRadius;
        ...
        public void LogSoftBodyInfo()
        {
            ...
            for (int i = 0; i < softBodies.Length; i++)
            {
                ...
                {
                    int nodeCount = softBodies[i].solver?.nodeManager?.NodeCount ?? 0;
        ...
        void OnValidate()
        {
            ...
            nodeGrabStiffness = Mathf.Clamp(nodeGrabStiffness, 1f, 50f);
            nodeGrabRadius = Mathf.Clamp(nodeGrabRadius, 0.1f, 2f);
        }

        public void FocusOnSoftBody(SoftBody softBody, float distance = 5f)
        {
            if (softBody == null || softBody.solver?.nodeManager == null) return;

            Vector3 center = Vector3.zero;
            int nodeCount = softBody.solver.nodeManager.NodeCount;

            foreach (var pos in softBody.solver.nodeManager.CurrentPositions)
            {
                center += pos;
            }
