...
namespace DynamicEngine
{
    ...
    [ExecuteInEditMode]
    public class DynaDesigner3D : MonoBehaviour
    {
        #region Enums
        ...
        // Beam Settings  (displayed/stored as stiffness = 1/compliance; converted on write to solver)
        public float defaultBeamStiffness = 100f;
        ...
        // Beam Bulk Multipliers (stored as percentages, 100 = no change)
        [HideInInspector] public bool showBeamMultipliers = false;
        public float beamStiffnessMultiplier = 100f;
        public float beamDampingMultiplier = 100f;
        public float beamPlastThreshMultiplier = 100f;
        public float beamPlastRateMultiplier = 100f;
        public float beamMaxDefMultiplier = 100f;
        ...
        public float nodeDisplaySize = 0.08f;
        public float beamLineThickness = 5.0f;
        ...
        public void LoadDefaultsFromTruss(Truss truss)
        {
            ...
            if (beams != null && beams.Count > 0)
            {
                ...
                // Convert from internal compliance to the designer's stiffness display value
                defaultBeamStiffness = firstBeam.compliance > 0f ? 1f / firstBeam.compliance : 100f;
        ...
        public void SyncDesignerToSolver()
        {
            SoftBody sb = GetComponent<SoftBody>();
            if (sb == null || sb.solver == null) return;

            // Update global solver settings
            sb.solver.defaultDamping = defaultBeamDamping;

            // Update individual beams if they already exist
            foreach (var beam in sb.solver.beams)
            {
                // Apply the designer's damping to the solver's active beams
                beam.damping = defaultBeamDamping * (beamDampingMultiplier / 100f);
                beam.compliance = (1f / defaultBeamStiffness) * (beamStiffnessMultiplier / 100f);
            }
        }
    }
}