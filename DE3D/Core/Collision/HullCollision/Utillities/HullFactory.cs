...
namespace Vella.UnityNativeHull
{
    public class HullFactory
    {
        ...
        public static unsafe NativeHull CreateFromTruss(Truss truss)
        {
            if (truss == null)
            {
                Debug.LogError("Truss is null");
                return default;
            }

            var trussFaces = truss.GetTrussFaces();
            var nodePositions = truss.NodePositions;

            if (trussFaces == null || trussFaces.Count == 0)
            {
                Debug.LogError("Truss has no faces");
                return default;
            }

            if (nodePositions == null || nodePositions.Length == 0)
            {
                Debug.LogError("Truss has no node positions");
                return default;
            }

            var faces = new List<DetailedFaceDef>();
            var verts = nodePositions.Select(v => RoundVertex(v)).ToArray();
            var uniqueVerts = verts.Distinct().ToList();

            // Create faces from truss faces
            foreach (var trussFace in trussFaces)
            {
                if (trussFace.nodeA >= nodePositions.Length ||
                    trussFace.nodeB >= nodePositions.Length ||
                    trussFace.nodeC >= nodePositions.Length)
                {
                    Debug.LogWarning($"Invalid face indices: {trussFace.nodeA}, {trussFace.nodeB}, {trussFace.nodeC}");
                    continue;
                }

                Vector3 p1 = verts[trussFace.nodeA];
                Vector3 p2 = verts[trussFace.nodeB];
                Vector3 p3 = verts[trussFace.nodeC];

                // Skip degenerate faces
                if (p1 == p2 || p2 == p3 || p3 == p1)
                {
                    Debug.LogWarning($"Skipping degenerate face: [{trussFace.nodeA}, {trussFace.nodeB}, {trussFace.nodeC}]");
                    continue;
                }
            ...
            if (faces.Count == 0)
            {
                Debug.LogError("No valid faces created from truss");
                return default;
            }
            ...
            // Remove vertices with no edges connected to them
        ...
        public unsafe static void SetFromFaces(ref NativeHull hull, NativeHullDef def)
        {
            ...
            var edgesList = new NativeHalfEdge[10000];
            var invalidFaces = new HashSet<int>();
            ...
            for (int i = 0; i < def.FaceCount; ++i)
            {
                ...
                var faceVerts = new HashSet<int>();
                for (int v = 0; v < vertCount; v++)
                {
                    if (!faceVerts.Add(vertices[v]))
                    {
                        Debug.LogWarning($"Face {i} has duplicate vertex {vertices[v]}, skipping face");
                        invalidFaces.Add(i);
                        break;
                    }
                }
                ...
                for (int j = 0; j < vertCount; ++j)
                {
                    ...
                    if (v1 == v2)
                    {
                        Debug.LogWarning($"Face {i} has degenerate edge ({v1}→{v2}), skipping face");
                        invalidFaces.Add(i);
                        break;
                    }
                    ...
                    if (edgeFound12)
                    {
                        ...
                        {
                            Debug.LogError($"Face {i} shares edge ({v1}→{v2}) with face {edgesList[e12].Face} in the same direction. " +
                                           $"This indicates non-manifold geometry. Skipping face {i}.");
                            invalidFaces.Add(i);
                            break;
                        }
            ...
            // Report invalid faces
            if (invalidFaces.Count > 0)
            {
                Debug.LogWarning($"Skipped {invalidFaces.Count} invalid faces due to non-manifold geometry. " +
                                $"Hull may not be watertight. Consider cleaning the source mesh.");
            }
        ...
        public struct PolygonPerimeter
        {
            ...
            public static List<Edge> CalculatePerimeter(int[] indices, ref List<float3> verts)
            {
                ...
                for (int i = 0; i < indices.Length - 1; i += 3)
                {
                    ...
                    {
                        Debug.LogWarning($"Skipping degenerate triangle: [{v1}, {v2}, {v3}]");
                        continue;
                    }
            ...
            private static List<Edge> Rebuild()
            {
                ...
                foreach (var edge in OutsideEdges)
                {
                    if (map.ContainsKey(edge.StartIndex))
                    {
                        Debug.LogWarning($"Duplicate edge start index detected: {edge.StartIndex}. Mesh may have non-manifold geometry.");
                        duplicates.Add(edge.StartIndex);
                    }
                    else
                    {
                        map[edge.StartIndex] = edge.EndIndex;
                    }
                }

                // Remove edges with duplicate start indices
                var validEdges = OutsideEdges.Where(e => !duplicates.Contains(e.StartIndex)).ToList();

                if (validEdges.Count == 0)
                {
                    Debug.LogError("No valid edges after removing duplicates. Mesh geometry is too degenerate.");
                    return OutsideEdges; // Return original edges as fallback
                }

                // Rebuild the edge chain
                var cur = validEdges.First().StartIndex;
                ...
                for (int i = 0; i < validEdges.Count; i++)
                {
                    ...
                    {
                        Debug.LogWarning($"Circular edge chain detected at index {cur}");
                        break;
                    }

                    if (!map.ContainsKey(cur))
                    {
                        Debug.LogWarning($"Edge chain broken at index {cur}");
                        break;
                    }
                ...
                return result.Count > 0 ? result : OutsideEdges;
            }
        }
    }
}