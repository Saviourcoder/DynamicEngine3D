...
namespace Vella.UnityNativeHull
{
    ...
    public class HullCollision
    {
        ...
        public static unsafe void QueryFaceDistance(out FaceQueryResult result, RigidTransform transform1, NativeHull hull1, RigidTransform transform2, NativeHull hull2)
        {
            ...
            quaternion invRot2 = math.inverse(transform2.rot);
            ...
            {
                NativePlane localPlane = hull1.GetPlane(i);

                // Transform normal once for support query in hull2's local space
                float3 localNormal = math.rotate(invRot2, math.rotate(transform1.rot, localPlane.Normal));
                float3 support = hull2.GetSupport(-localNormal);

                // Calculate distance in transformed space
                NativePlane transformedPlane = transform * localPlane;
                float distance = transformedPlane.Distance(support);
