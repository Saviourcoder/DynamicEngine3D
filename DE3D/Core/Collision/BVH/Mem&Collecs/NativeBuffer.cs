...
namespace Vella.Common
{
    ...
    public static class NativeSortExtension2
    {
        ...
        unsafe public static void Sort<T, U>(this NativeBuffer<T> array, U comp) where T : struct where U : IComparer<T>
        {
            ...
            IntroSort<T, U>(array.GetUnsafePtr(), 0, array.Length - 1, 2 * (int)math.log2(array.Length), comp);
        }
    ...
    (void*)((IntPtr)m_Buffer + UnsafeUtility.SizeOf<T>() * index)
);
        ...
        {
            return ref UnsafeUtility.AsRef<T>(
    (void*)((IntPtr)m_Buffer + (UnsafeUtility.SizeOf<T>() * index) + offset)
);
