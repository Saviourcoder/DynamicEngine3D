...
namespace Vella.Common
{
    ...
    public unsafe class NativeBoundingHierarchy<T> : IDisposable where T : struct
    {
        public delegate bool NodeTest(BoundingBox box);
        public delegate int GetId(T item);
        public delegate float3 GetPosition(T item);
        public delegate float GetRadius(T item);
        ...
        private GetId _getId;
        private GetPosition _getPosition;
        private GetRadius _getRadius;
        ...
        private NodePositionAxisComparer _axisComparer;

        private readonly NodeDepthComparer _nodeDepthComparer;

        private NativeBoundingHierarchy() { }

        public NativeBoundingHierarchy(GetId getId, GetPosition getPosition, GetRadius getRadius, List<T> objects = null, int maxPerLeft = 1)
        {
            _getId = getId;
            ...
            _map = new NativeHashMap<int, Node>(MaxNodes, Allocator.Persistent);
            ...
            _axisComparer = new NodePositionAxisComparer { getPosition = _getPosition };
        ...
        {
            return _map.TryGetValue(_getId(item), out node);
        }
        ...
        {
            _axisComparer.axis = axis;
            split.Items.Sort(_axisComparer);
        ...
        {
            var box = BoundingBox.FromSphere(_getPosition(newOb), _getRadius(newOb));
        ...
        public bool TryFindBetterNode(Node* currentNode, T item, out Node* node)
        {
            ...
            var box = BoundingBox.FromSphere(_getPosition(item), _getRadius(item));
        ...
        internal void RemoveItemFromNode(Node* node, T newOb)
        {
            ...
            int idx = -1;
            int targetId = _getId(newOb);
            for (int i = 0; i < bucket.Length; i++)
            {
                if (_getId(bucket[i]) == targetId)
                {
                    idx = i;
                    break;
                }
        ...
        internal void ComputeVolume(Node* node)
        {
            ...
            AssignVolume(node, _getPosition(bucket[0]), _getRadius(bucket[0]));

            for (var i = 0; i < bucket.Length; i++)
            {
                ExpandVolume(node, _getPosition(bucket[i]), _getRadius(bucket[i]));
            }
        }
        ...
        /// </summary>
        public struct NodePositionAxisComparer : IComparer<T>
        {
            public Axis axis;
            ...
            public int Compare(T a, T b)
            {
                ...
                return axis switch
                {
                    Axis.X => posA.x.CompareTo(posB.x),
                    Axis.Y => posA.y.CompareTo(posB.y),
                    Axis.Z => posA.z.CompareTo(posB.z),
                    _ => 0
                };
            }
        }
