...
namespace DynamicEngine
{
    ...
    {
        // ── Profiler Markers ────────────────────────────────────────────────────
        private static readonly ProfilerMarker s_PerfSolve = new ProfilerMarker("Solver.Solve");
        private static readonly ProfilerMarker s_PerfIntegrate = new ProfilerMarker("Solver.Integrate");
        private static readonly ProfilerMarker s_PerfSolveConstraints = new ProfilerMarker("Solver.SolveConstraints");
        private static readonly ProfilerMarker s_PerfResolveCollisions = new ProfilerMarker("Solver.ResolveCollisions");
        private static readonly ProfilerMarker s_PerfFinalizePositions = new ProfilerMarker("Solver.FinalizePositions");
        private static readonly ProfilerMarker s_PerfApplyPressure = new ProfilerMarker("Solver.ApplyPressure");
        private static readonly ProfilerMarker s_PerfSolveCrossBody = new ProfilerMarker("Solver.SolveCrossBody");
        private static readonly ProfilerMarker s_PerfSolveCrossBodyBeam = new ProfilerMarker("Solver.SolveCrossBodyBeam");
        private static readonly ProfilerMarker s_PerfSolveEdgeSliding = new ProfilerMarker("Solver.SolveEdgeSliding");
        private static readonly ProfilerMarker s_PerfCheckAndBreakConstraints = new ProfilerMarker("Solver.CheckAndBreakConstraints");
        private static readonly ProfilerMarker s_PerfApplyPlasticity = new ProfilerMarker("Solver.ApplyPlasticity");
        private static readonly ProfilerMarker s_PerfDeformMesh = new ProfilerMarker("Solver.DeformMesh");
        private static readonly ProfilerMarker s_PerfUpdateNodeRotations = new ProfilerMarker("Solver.UpdateNodeRotations");
        private static readonly ProfilerMarker s_PerfGenerateNodesAndBeams = new ProfilerMarker("Solver.GenerateNodesAndBeams");
        private static readonly ProfilerMarker s_PerfClearExistingNodes = new ProfilerMarker("Solver.ClearExistingNodes");
        private static readonly ProfilerMarker s_PerfGenerateNodes = new ProfilerMarker("Solver.GenerateNodes");
        private static readonly ProfilerMarker s_PerfGenerateBeamsFromArray = new ProfilerMarker("Solver.GenerateBeamsFromArray");
        private static readonly ProfilerMarker s_PerfGenerateBeamsFromDist = new ProfilerMarker("Solver.GenerateBeamsFromDistance");
        private static readonly ProfilerMarker s_PerfGenerateCubeTest = new ProfilerMarker("Solver.GenerateCubeTest");
        private static readonly ProfilerMarker s_PerfInitializeFromTruss = new ProfilerMarker("Solver.InitializeFromTruss");
        private static readonly ProfilerMarker s_PerfRestoreNodesAndBeams = new ProfilerMarker("Solver.RestoreNodesAndBeams");
        private static readonly ProfilerMarker s_PerfApplyForceToNode = new ProfilerMarker("Solver.ApplyForceToNode");
        private static readonly ProfilerMarker s_PerfApplyWorldForceToNode = new ProfilerMarker("Solver.ApplyWorldForceToNode");
        private static readonly ProfilerMarker s_PerfApplyImpulse = new ProfilerMarker("Solver.ApplyImpulse");
        private static readonly ProfilerMarker s_PerfFindClosestNodeToRay = new ProfilerMarker("Solver.FindClosestNodeToRay");
        // ────────────────────────────────────────────────────────────────────────
        ...
        private int[] _beamColours;               // colour index per beam
        private List<int>[] _colourGroups;        // beam indices per colour
        ...
        {
            using var _ = s_PerfInitializeFromTruss.Auto();
            _trussAsset = truss;
            if (truss != null)
            {
        ...
        {
            using var _ = s_PerfGenerateNodesAndBeams.Auto();
        ...
        {
            using var _ = s_PerfClearExistingNodes.Auto();
            nodeManager.Clear();
            beams.Clear();
            nodeMasses.Clear();
        ...
        {
            using var _ = s_PerfGenerateNodes.Auto();
        ...
        {
            using var _ = s_PerfGenerateBeamsFromArray.Auto();
        ...
        {
            using var _ = s_PerfGenerateBeamsFromDist.Auto();
            for (int i = 0; i < positions.Length; i++)
            {
        ...
        {
            using var _ = s_PerfGenerateCubeTest.Auto();
        ...
        public void SolveSubstep(float subDt)
        {
            if (nodeManager == null || nodeManager.NodeCount == 0 || _owner == null || beams == null) return;

            // Apply pre-integration forces
            ApplyPressure(subDt);
            ApplyBeamDampingForces(subDt);
            ...
            ApplyPlasticity(subDt, worldRestLengthScale);
            CheckAndBreakConstraints(subDt);

            collisionHandler.ResolveCollisions(nodeManager, _owner, _matterAsset, nodeMasses, subDt, 0.005f, visualizeForces);
            FinalizePositions();
        }
        ...
        internal void Integrate(float dt)
        {
            ...
            Vector3 worldGravity = Vector3.down * 9.81f; // Standardized gravity
            ...
            float dampingFactor = Mathf.Exp(-velocityDamping * dt);
        ...
        internal void SolveConstraints(float dt)
        {
            ...
            // FIXED: Single job executes all iterations. No data races. 0 Scheduling overhead.
            var job = new ConstraintsJob
            {
                iterations = constraintIters,
        ...
        private void SolveCrossBody(float dt)
        {
            using var _ = s_PerfSolveCrossBody.Auto();
        ...
        {
            using var _ = s_PerfSolveCrossBodyBeam.Auto();
            ...
            if (hasMinLimit && currentLength < beam.minLength) targetLength = beam.minLength;
            else if (hasMaxLimit && currentLength > beam.maxLength) targetLength = beam.maxLength;
        ...
        {
            using var _ = s_PerfSolveEdgeSliding.Auto();
        ...
        private void CheckAndBreakConstraints(float dt)
        {
            using var _ = s_PerfCheckAndBreakConstraints.Auto();
        ...
        {
            using var _ = s_PerfResolveCollisions.Auto();
        ...
        {
            using var _ = s_PerfApplyPressure.Auto();
        ...
        private void ApplyBeamDampingForces(float dt)
        {
            float dtSq = dt * dt;
            for (int i = 0; i < beams.Count; i++)
            {
                Beam beam = beams[i];
                if (!beam.isActive || beam.isEdgeSliding) continue;

                Solver solverA = beam.IsCrossBody ? beam.bodyA.solver : this;
                Solver solverB = beam.IsCrossBody ? beam.bodyB.solver : this;

                int nA = beam.nodeA;
                int nB = beam.nodeB;

                Vector3 posA = solverA.nodeManager.CurrentPositions[nA];
                Vector3 posB = solverB.nodeManager.CurrentPositions[nB];
                Vector3 prevA = solverA.nodeManager.PreviousPositions[nA];
                Vector3 prevB = solverB.nodeManager.PreviousPositions[nB];

                // Calculate current velocities
                Vector3 vA = (posA - prevA) / dt;
                Vector3 vB = (posB - prevB) / dt;
                ...
                // Calculate relative velocity along the beam (positive means compressing)
                float relVel = Vector3.Dot(vA - vB, dir);

                // Calculate damping force (F = -c * v)
                float effectiveDamping = beam.damping;
                if (beam.IsCrossBody && (beam.minLength > 0f || !float.IsPositiveInfinity(beam.maxLength)))
                {
                    effectiveDamping *= 2.5f; // Keep original motor multiplier
                }

                Vector3 dampForce = -effectiveDamping * relVel * dir;

                // Apply forces explicitly as accelerations to the Verlet previous positions
                if (!solverA.nodeManager.IsPinned[nA])
                {
                    float massA = solverA.nodeMasses[nA];
                    solverA.nodeManager.PreviousPositions[nA] -= (dampForce / massA) * dtSq;
                }

                if (!solverB.nodeManager.IsPinned[nB])
                {
                    float massB = solverB.nodeMasses[nB];
                    solverB.nodeManager.PreviousPositions[nB] += (dampForce / massB) * dtSq;
                }
            }
        }
        ...
        {
            using var _ = s_PerfApplyForceToNode.Auto();
        ...
        {
            using var _ = s_PerfApplyWorldForceToNode.Auto();
            if (!IsValidNodeForForce(nodeIndex)) return;
        ...
        {
            using var _ = s_PerfApplyImpulse.Auto();
        ...
        {
            using var _ = s_PerfFindClosestNodeToRay.Auto();
            int closestNode = -1;
        ...
        {
            using var _ = s_PerfDeformMesh.Auto();
        ...
        {
            using var _ = s_PerfUpdateNodeRotations.Auto();
            // OPTIMIZATION: In a production build, you should cache this neighbor 
            // list in a NativeArray instead of rebuilding a List<List<int>>.
            int nodeCount = nodeManager.NodeCount;
            var neighbors = BuildNeighborLists(nodeCount);
        ...
        {
            using var _ = s_PerfRestoreNodesAndBeams.Auto();
            if (transform == null) return;
        ...
        {
            using var _ = s_PerfApplyPlasticity.Auto();
            for (int beamIndex = 0; beamIndex < beams.Count; beamIndex++)
            {
                ...
                // FIX: Use absolute strain so compression (negative strain) triggers plasticity
