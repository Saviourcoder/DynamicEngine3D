...
namespace DynamicEngine
{
    ...
    public class Beam
    {
        ...
        {
            return bodyA?.solver?.nodeManager?.GetPosition(nodeA) ?? Vector3.zero;
        }

        public Vector3 GetNodeBWorldPosition()
        {
            return bodyB?.solver?.nodeManager?.GetPosition(nodeB) ?? Vector3.zero;
        }

        public Vector3 GetNodeALocalPosition()
        {
            if (bodyA?.solver?.nodeManager == null) return Vector3.zero;
            var list = bodyA.solver.nodeManager.PredictedPositions;
            return (nodeA >= 0 && nodeA < list.Count) ? list[nodeA] : Vector3.zero;
        }

        public Vector3 GetNodeBLocalPosition()
        {
            if (bodyB?.solver?.nodeManager == null) return Vector3.zero;
            var list = bodyB.solver.nodeManager.PredictedPositions;
            return (nodeB >= 0 && nodeB < list.Count) ? list[nodeB] : Vector3.zero;
        }

        public bool AreNodesValid()
        {
            bool validA = bodyA?.solver?.nodeManager != null && nodeA >= 0 && nodeA < bodyA.solver.nodeManager.NodeCount;
            bool validB = bodyB?.solver?.nodeManager != null && nodeB >= 0 && nodeB < bodyB.solver.nodeManager.NodeCount;
            return validA && validB;
        }

        public bool IsNodeAPinned()
        {
            if (bodyA?.solver?.nodeManager == null) return false;
            var list = bodyA.solver.nodeManager.IsPinned;
            return (nodeA >= 0 && nodeA < list.Count) && list[nodeA];
        }

        public bool IsNodeBPinned()
        {
            if (bodyB?.solver?.nodeManager == null) return false;
            var list = bodyB.solver.nodeManager.IsPinned;
            return (nodeB >= 0 && nodeB < list.Count) && list[nodeB];
        }
        ...
        private Vector3 GetNodeAVelocity(float deltaTime)
        {
            ...
            if (bodyA?.solver?.nodeManager == null) return Vector3.zero;
            var prevList = bodyA.solver.nodeManager.PreviousPositions;

            Vector3 prev = (nodeA >= 0 && nodeA < prevList.Count) ? prevList[nodeA] : curr;
        ...
        private Vector3 GetNodeBVelocity(float deltaTime)
        {
            ...
            if (bodyB?.solver?.nodeManager == null) return Vector3.zero;
            var prevList = bodyB.solver.nodeManager.PreviousPositions;

            Vector3 prev = (nodeB >= 0 && nodeB < prevList.Count) ? prevList[nodeB] : curr;
            return (curr - prev) / deltaTime;
        }

        private void UpdateNodeAPosition(Vector3 correction)
        {
            if (bodyA?.solver?.nodeManager == null) return;
            var list = bodyA.solver.nodeManager.PredictedPositions;

            if (nodeA >= 0 && nodeA < list.Count)
                list[nodeA] += correction;
        }

        private void UpdateNodeBPosition(Vector3 correction)
        {
            if (bodyB?.solver?.nodeManager == null) return;
            var list = bodyB.solver.nodeManager.PredictedPositions;

            if (nodeB >= 0 && nodeB < list.Count)
                list[nodeB] += correction;
        }
