...
namespace DynamicEngine
{
    public class SoftBody : MonoBehaviour
    {
        ...
        [SerializeField] public bool showNodesAndLinks = false;
        [SerializeField] public bool showNodes = true;
        [SerializeField] public bool showLinks = true;
        [SerializeField] public bool showNodeIndices = false;
        ...
        [SerializeField] public Color nodeColor = Color.green;
        [SerializeField] public Color pinnedNodeColor = Color.red;
        [SerializeField] public Color linkColor = Color.blue;
        [SerializeField] public Color stretchedLinkColor = Color.red;
        [SerializeField] public Color compressedLinkColor = Color.yellow;
        [SerializeField] public Color influenceRadiusColor = new Color(1f, 1f, 0f, 0.3f);
        ...
        private void Awake()
        {
            ...
            if  (truss == null)
            truss = GetComponent<NodeLinkEditor>()?.SoftBody?.truss;

            if  (truss == null)
                Debug.LogError("No TrussAsset assigned and none found on NodeLinkEditor!", this);
        }
        ...
        void Update()
        {
            ...
            if (showNodesAndLinks && solver != null)
            {
        ...
        public void ApplyTruss()
        {
            ...
            // 🎯 CRITICAL: Apply material properties from TrussAsset
        ...
        private void DrawNodes()
        {
            for (int i = 0; i < solver.nodeManager.Nodes.Count; i++)
            {
                ...
                // Draw node index if enabled (using Debug.DrawRay for runtime compatibility)
                if (showNodeIndices)
                {
                    Vector3 labelPos = position + Vector3.up * (nodeDisplaySize + 0.1f);
                    // Draw a small upward ray to indicate node index (visible in Scene view)
                    Debug.DrawRay(labelPos, Vector3.up * 0.1f, color, Time.fixedDeltaTime);
                    
                    // For runtime, you can see node indices in the node GameObjects names in hierarchy
                    if (Application.isPlaying && solver.nodeManager.Nodes[i] != null)
                    {
                        solver.nodeManager.Nodes[i].name = $"Node_{i}";
                    }
                }
            }
        }
        ...
        private void DistributeMassEvenly()
        {
            ...
            // Update the NodeLinkEditor's nodeMass to match our distribution
            var nodeLinkEditor = GetComponent<NodeLinkEditor>();
