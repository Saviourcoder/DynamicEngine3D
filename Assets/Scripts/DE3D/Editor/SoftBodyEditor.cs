...
namespace DynamicEngine
{
    ...
    public class SoftBodyEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUI.BeginChangeCheck();
            float newScaledMass = EditorGUILayout.FloatField(new GUIContent("Mass Scale", "Total mass distributed across all nodes"), softBody.ScaledMass);

            // Add disabled field for Mass Scale Multiplier
            EditorGUI.BeginDisabledGroup(true);
            EditorGUILayout.FloatField(new GUIContent("Scaled Mass", "Multiplier applied to the original Truss masses at runtime"), softBody.MassScale);
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (softBody.solver?.nodeManager?.Nodes != null && softBody.solver.nodeManager.Nodes.Count > 0)
                {
                    ...
                    Debug.Log($"SoftBody: Mass updated - {newScaledMass:F2}kg total (Scale: {softBody.MassScale:F2}x).");
                }
            }
